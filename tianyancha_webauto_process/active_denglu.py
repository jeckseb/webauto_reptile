...
from UI_active_denglu import Ui_DengluWindow
from reptile_process import ProcessWindow
...
# 登录页面
class DengluWindow(QMainWindow, Ui_DengluWindow):

    def __init__(self, company_list, company_name_sum, generate_name, generate_data):
        super(DengluWindow, self).__init__()
        self.setupUi(self)
    ...
    def tuokuai_choose_run(self):
        if self.auto_Bracket == True:
            ...
            try:
                self.driver.find_element_by_xpath('/html/body/div[1]/div/div[1]/div[1]/div[1]/div/div[2]/div/div[5]/a')
            except:
                try:
    ...
    def for_tuokuai(self):
        ...
        for i in range(5):
            try:
                ...
                print('按钮移动距离：', self.track)
                self.get_track_move(self.track)
                self.driver.find_element_by_xpath('/html/body/div[1]/div/div[1]/div[1]/div/div/div[2]/div/div[5]/a')
                self.j += 1
                return self.track
            except:
                print('拖块误差错误，重试中...')
