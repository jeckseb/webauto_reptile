...
class MyThread(QThread):
    ...
    def __init__(self, company_list, driver, generate_name, generate_data, company_name_sum, url):
    ...
    def active(self):
        ...
        for company_name in self.company_list:
            if u'\u4e00' <= company_name <= u'\u9fff':
                ...
                if a == b:
                    input1 = self.driver.find_element_by_id('searchkey')
                    time.sleep(1)
                    ...
                    time.sleep(1)
                    input2 = self.driver.find_element_by_id('headerKey')
                    for i in company_name:
                        input2.send_keys(i)
                        time.sleep(random.uniform(0.2, 0.5))
                    self.driver.find_element_by_xpath('/html/body/header/div/form/div/div/span/button').click()
                    time.sleep(1)
                    self.process_browser_window.emit("正在查询该企业，请稍后。。。")
                    self.get_company(company_name)
            else:
    ...
    def get_company(self, company_name):
        ...
        time.sleep(0.5)
        js1 = "window.scrollTo(0,350)"
        self.driver.execute_script(js1)
        time.sleep(1)
        try:
            self.driver.find_element_by_xpath(
                '/html/body/div[4]/div/div[1]/div[3]/section/table/tbody/tr[1]/td[3]/a').click()
        except:
            self.process_browser_window.emit("爬取该企业失败，请检查该企业名称是否正确！")
            self.s += 1
            self.success_browser_num.emit(self.s)
            self.n += 1
            self.end_browser_num.emit(self.n)
        ...
        company_data_1 = ''
        try:
            business_content = self.driver.find_elements_by_xpath('/html/body/div[4]/div[2]/div/div[2]/section[3]/table/tbody/tr')
            # 工商信息采集
            for cont_1 in business_content:
                ...
                try:
                    ...
                    pass
                try:
                    company_title_3 = cont_1.find_element_by_xpath('.//td[5]')
                except:
                    pass
                try:
                    company_info_3 = cont_1.find_element_by_xpath('.//td[6]')
                except:
                    pass
                company_data_1 += company_title_1.text + ':' + company_info_1.text + ';' + company_title_2.text + \
                                  ':' + company_info_2.text + company_title_3.text + ':' + company_info_3.text + '\n'
        ...
        element = self.driver.find_element_by_id('base_title')
        Action.move_to_element(element).perform()
        time.sleep(1)
        company_data_2 = ''
        try:
            self.driver.find_element_by_xpath(
                '/html/body/div[4]/div[2]/div/header/div[2]/div/div[1]/div/a[4]').click()
            time.sleep(1)
            shareholder_content = self.driver.find_elements_by_xpath(
                '/html/body/div[4]/div[2]/div/div[2]/section[5]/div[3]/table/tbody/tr[2]')
            # 股东信息采集
            for cont_2 in shareholder_content:
                shareholder_info_1 = cont_2.find_element_by_xpath('.//td[2]/table/tbody/tr/td[2]/a/span')
                shareholder_info_2 = cont_2.find_element_by_xpath('.//td[3]')
                shareholder_info_3 = cont_2.find_element_by_xpath('.//td[4]')
                shareholder_info_4 = cont_2.find_element_by_xpath('.//td[5]')
                company_data_2 += shareholder_info_1.text + ';' + shareholder_info_2.text + ';' + shareholder_info_3.text + shareholder_info_4.text + '\n'
            self.process_browser_window.emit(company_name + '--股东信息爬取成功!')
            company_content_2 = '--股东信息：' + '\n' + '股东-' + '-持股占比-' + '-认缴出资额-' + '-认缴出资日期' + '\n' + company_data_2
        except:
            company_content_2 = '--股东信息：' + '\n' + '股东-' + '-持股占比-' + '-认缴出资额' + '-认缴出资日期' + '\n' + company_data_2
        ...
        try:
            self.driver.find_element_by_xpath(
                '/html/body/div[4]/div[2]/div/header/div[2]/div/div[1]/div/a[7]').click()
            time.sleep(1)
            manager_content = self.driver.find_elements_by_xpath(
                '/html/body/div[4]/div[2]/div/div[2]/section[7]/div[2]/table/tbody/tr[2]')
            # 公司主要人员信息采集
            for cont_3 in manager_content:
                manager_info_1 = cont_3.find_element_by_xpath('.//td[2]/table/tbody/tr/td[2]/a/span')
                manager_info_2 = cont_3.find_element_by_xpath('.//td[3]')
                manager_info_3 = cont_3.find_element_by_xpath('.//td[4]')
                manager_info_4 = cont_3.find_element_by_xpath('.//td[5]')
                company_data_3 += manager_info_1.text + ';' + manager_info_2.text + manager_info_3.text + ';' + manager_info_4.text + '\n'
...
class ProcessWindow(QMainWindow, Ui_ProcessWindow):
    ...
    def thread_author(self):
        self.thread = MyThread(self.company_list, self.driver, self.generate_name, self.generate_data, self.company_name_sum, self.url)
